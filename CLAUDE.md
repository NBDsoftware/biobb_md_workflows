...
Each CLI should accept a `--config <yaml>` to override the auto-generated default config and for reproducibility purposes.

## Architecture

All four workflows share the same shape, built around `biobb_common`:

1. **YAML config-driven steps.** Each workflow defines a `config_contents()` returning a default YAML string, written by `create_config_file()` if the user doesn't pass `--config`. The YAML has a `global_properties` block (`working_dir_path`, `restart`, `remove_tmp`, `can_write_console_log`) and one block per step named `stepN_name` (or prefixed `sectionprefix_stepN_name`, see below), each with a `tool`, a `paths` dict, and a `properties` dict.
...
3. **Workflow function body.** The main function (`protein_preparation()`, `ligand_parameterization()`, `md_gromacs()`, `traj_postprocessing()`) reads the config, then calls each BioBB building block in sequence as `tool_fn(**global_paths["stepN"], properties=global_prop["stepN"])`, mutating `global_paths`/`global_prop` in between when CLI args need to override the YAML (e.g. injecting `input_pdb_path`, chain lists, pH, forcefield). Steps are logged individually via `global_log`.
